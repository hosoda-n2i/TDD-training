...
- **`{{DB}}` / `{{AUTH}}` が「なし」のプロジェクトでは、条件節を機械置換で残さず「対象外」へ畳む。** テンプレには `### DB テスト（{{DB}} がある場合）`・`- {{DB}} がある場合、…`・`- {{AUTH}} がある場合、…`（`testing.md` / `agents/tdd-guide.md`）のような条件付き見出し・箇条書きと、DB/認証スニペット（`{{DB_TEST_UTILS_SNIPPET}}` 等）がある。単純置換すると「**なし がある場合**」のような壊れた文や未展開スニペットが残る。なしの場合はこれらの節を削るか「対象外（DB / 認証なし）」の 1 行に置き換え、E2E は素の `@playwright/test` を使う形に直す。
