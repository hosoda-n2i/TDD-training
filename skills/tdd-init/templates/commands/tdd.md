...
- 引数に `.claude/tdd/specs/*.md`（EARS 仕様）パスが渡された／対応する spec が存在する場合は、それを**構造化された受け入れ条件の参照**として読み込み、REQ ごとにテストを割り当てる。ただし **spec はゲートではなく、テストが acceptance の実体**。spec と実テストがずれたらテストを正として spec を直す。spec が無くても機能説明から実装できる。
