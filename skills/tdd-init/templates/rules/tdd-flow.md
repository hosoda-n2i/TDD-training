...
- **SCAFFOLD**: 対象関数 / コンポーネントの型と `throw new Error('Not implemented')` スタブを先に書く。テストが import するシグネチャを確定させる。
- **RED**: 1 受け入れ条件 = 1 テスト。Arrange–Act–Assert。
- **SPEC-CHECK**: 書いたテストが対応する仕様（EARS REQ / 受け入れ条件）と意味的に一致しているか確認する（仕様↔テスト整合のみ。spec が無ければスキップ）。
- **GREEN**: 最小実装で通す。早すぎる抽象化をしない。
- **REFACTOR**: 緑のまま整える。テストを毎回走らせる。
...
- 既存テストが回帰していない（全 unit / integration 緑）
...
FAIL や生存ミュータントが出たら、指された フェーズ（`/spec`・`/e2e`・`/tdd`）に戻って直してから「完了」とする。

> **SPEC-CHECK と `/harden` の住み分け**: SPEC-CHECK（仕様↔テスト整合・RED 直後・緑化前）と `/harden`（テスト自体の正しさ・強さ・受け入れ条件が緑になってから）は別物。混同しない。
