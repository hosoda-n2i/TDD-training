...
- **SPEC-CHECK（RED→GREEN 間 / 緑化前）**: 書いたテストが対応 REQ・受け入れ条件と意味的に一致するかだけを見る（trigger→response の写し / 期待値が仕様由来か / REQ↔テストの過不足）。テストの強さ・網羅・実装の正しさは見ない（→ `/harden`・`/adversary`・GREEN）。spec が無ければスキップ。ずれたらテストを仕様へ直す（実装に合わせない）。
...
FAIL や生存ミュータントが出たら、指されたフェーズ（`/spec`・`/e2e`・`/tdd`）に戻って直してから「完了」とする。
