...
# Dual-loop TDD の規律 — sample-nextjs
...
- **COVERAGE**: `npm run coverage` で 80%+ を確認。届かなければ未カバーの分岐 / 異常系を追加。
...
FAIL や生存ミュータントが出たら、指されたフェーズ（`/spec`・`/e2e`・`/tdd`、または refactor ステップ）に戻って直してから「完了」とする。
